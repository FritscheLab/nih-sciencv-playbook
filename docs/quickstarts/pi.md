...
{: .note }
> For an annual RPPR, the separate MFTRP statement in Section G.1 is an additional requirement for every senior/key person. It is not part of the SciENcv PDF. See [Research-security certifications]({{ site.baseurl }}{% link policy/research-security-certifications.md %}).
...
flowchart TD
    ...
    B --> C["Select up to 5 + up to 5 Products"]
    C --> D["Draft narratives and honors"]
    D --> E["Check RST timing for an application"]
    E --> F{"CPOS requested now?"}
    F -- "Yes" --> G["Build CPOS in SciENcv"]
    F -- "No" --> H["Track for JIT, RPPR, or Prior Approval"]
    G --> I["Certify and download required PDFs"]
    H --> I
    I --> J["Keep Common Forms unmodified by default"]
```
